...
def multiple_seeds_to_csv_no_mitral(seedsfile, arrayname, labels, outfile):
    ...
    for l in labels:
        ...
        line_binary=' '.join(format(ord(c), 'b') for c in line)
        f.write(line.encode())
...
def clip_vein_endpoint(surface, ifile_sufix, targetdistance,
    ...
    for k in range(1,nseeds+1):
        ...
        while ((currentabscissa < targetdistance) and
            ...
            # error here means that there's not enough length for the pv clipping
            # retry with other seeds
...
def cylinder_mitral_clip(surface, surfaceorig, vis=False, dist=10, arrayname='', arrayvalue=''):
    ...
    radius = euclideandistance(closest_point,center)+5 # Hint: adjust radius if mitral clipping is too high or too low

    vectordown = multiplyvector(normal,2)
...
def original2unfold_point_map(point_map):
    ...
    # turning list of tuples in two list
    std2average = list(zip(*point_map['std2average']))
    average2unfold = list(zip(*point_map['average2unfold']))
